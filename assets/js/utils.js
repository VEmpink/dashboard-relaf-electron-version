...
/**
 * Electron API `ipcRenderer.send()`
 * @param {string} channel 
 * @param {*} [data] 
 */
function ipcRequest(channel, data) {
    window.request.toServer(channel, data);
}

/**
 * Electron API `ipcRenderer.once()`
 * @param {string} channel 
 * @param {Function} callback 
 */
function ipcReceive(channel, callback) {
    window.receive.fromServer(channel, callback);
}
...
function isValidDataCustomer(obj) {
    ...
    const allowedObjProp = listAllowedObjProp();
...
/**
 * Metode untuk mencari atau mengambil data yang ada di `indexedDB` dengan metode `.openCursor()`
 * @param {*} [id] - Data Id
 */
function getDataCustomer(id) {
    return new Promise(function (resolve, reject) {
        userdb().then(function(db) {
            const transaction = db.transaction('customer', 'readonly');
            const getData = transaction.objectStore('customer').openCursor(id);
            ...
            getData.onsuccess = function(e) {
                ...
                if (result) {
                    dataCustomer.push(result.value);
                    result.continue();
                }
            };
...
/**
 * Metode untuk menambah/mengubah data customer, jika `keyPath: "date"` (Id) belum ada di `indexedDB`
 * data secara otomatis akan ditambahkan, tapi jika data yang ditambahkan Idnya sama dengan yang
 * ada di `indexedDB` maka data tersebut secara otomatis hanya akan terubah
 * @param {object} obj
 */
function putDataCustomer(obj) {
    return new Promise(function(resolve, reject) {
        userdb().then(function(db) {
            if (isObject(obj) && isValidDataCustomer(obj)) {
                obj = filterAllowedObjProp(obj);
        
                const transaction = db.transaction('customer', 'readwrite');
                const dataCustomer = transaction.objectStore('customer').put(obj);
                
                dataCustomer.onerror = function(e) {
                    reject(e);
                };
...
/**
 * Metode untuk menghapus data di `indexedDB`
 * @param {array} unixId 
 */
function delDataCustomer(unixId) {
    return new Promise(function(resolve, reject) {
        userdb().then(function(db) {
            const transaction = db.transaction('customer', 'readwrite')
            const delData = transaction.objectStore('customer').openCursor(unixId);
            
            delData.onerror = function(e) {
                reject(e);
            };

            delData.onsuccess = function(e) {
                const result = e.target.result;
...
function getDatatable(dataPerPagination, 
    ...
    getDataCustomer().then(function(dataCustomer) {
        ...
        console.error(err);
        showToastsMsg('error', 'Gagal menampilkan datatable!');
    });
}
